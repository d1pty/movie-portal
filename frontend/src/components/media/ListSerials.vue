<template>
	...
	<div class="block_search">
		<input v-model="searchQuery" @input="searhSerial" type="text" placeholder="Поиск..." class="search">
    </div>
	<ul>
  ...
  export default {
	...
	getMedias() {
		http
		.get("/ListMedias/2?page="+this.currentPage)
		.then(response => {
	...
	searhSerial() {
		if (this.searchQuery) {
		http
		.get("/media/title/"+this.searchQuery+"/2")
		.then(response => {
			this.medias = response.data;
			const requests = this.medias.map(media => {
			const yearPromise = http.get("/year/" + media.year_id);
